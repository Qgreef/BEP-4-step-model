...
r2a = Road(G, "a", "b", 2.75, 3600)
r2b = Road(G, "b", "a", 2.75, 3600)
...
ftt_data = pd.DataFrame(ftt_matrix, columns=labels, index=labels)
...
for i in range(1):
    ...
    (att_matrix[9, 10], att_matrix[10, 9], att_matrix[9, 11], att_matrix[11, 9], att_matrix[10, 11], att_matrix[11, 10],
     att_matrix[16, 17], att_matrix[17, 16]) = (10 ** 100, 10 ** 100, 10 ** 100, 10 ** 100,
                                                10 ** 100, 10 ** 100, 10 ** 100, 10 ** 100,)
    att_data = pd.DataFrame(att_matrix, columns=labels, index=labels)
    print("loop", i)
...
nx.draw(G, width=intensities, with_labels=True)
#plt.show()
